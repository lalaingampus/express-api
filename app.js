...
app.post('/data_hutang', authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    const docRef = await db.collection('data_hutang').add({
      userId,            // Simpan userId untuk query nanti
      debtToPay,
      keterangan,
      createdAt: new Date().toISOString(),
    });
    ...
    console.error('Error menambahkan data hutang:', error);
...
app.get('/data_hutang', authenticateJWT, async (req, res) => {
  ...
  try {
    const snapshot = await db.collection('data_hutang')
    .where('userId', '==', userId)
    // .orderBy('createdAt', 'desc') // hapus sementara
    .get();
    ...
    res.status(200).json({
      message: 'Daftar data hutang berhasil diambil.',
      data: hutangList
    });
  } catch (error) {
    console.error('Error mengambil data hutang:', error);
